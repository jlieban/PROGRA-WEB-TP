...
export default function ModalCarrito({ abierto, carrito, onClose, onEliminar, onActualizarCantidad, onComprar }) {
    useEffect(() => {
                    ...
                    {carrito.length === 0 ? (
                        ...
                        carrito.map(item => (
                            <div key={item.id} className="carrito-item">
                                ...
                                <p className="carrito-item-precio">${(item.precio * item.cantidad).toLocaleString('es-AR')}</p>
                            </div>
                        ))
                    )}
                </div>
